package by.epam.jwd.testingApp.controller.mapping;

public final class AttributeNames {

    public static final String USER_ROLE = "userRole";
    public static final String USER_ID = "userID";
    public static final String REMEMBER_USER = "remember";
    public static final String EMAIL = "email";
    public static final String PASSWORD = "password";
    public static final String NICK_NAME = "nickName";
    public static final String PASSED_USERS = "passed";

    public static final String STATEMENT = "statement";
    public static final String CORRECT_STATEMENT_LIST = "correct";
    public static final String STATEMENT_ID = "statementId";
    public static final String STATEMENT_LIST = "statementsList";
    public static final String CHOSEN_STATEMENTS_LIST = "chosenStatements";//

    public static final String QUESTION_NAME = "questionName";
    public static final String QUESTION_ID = "questionId";
    public static final String QUESTION_LIST = "questions";
    public static final String QUESTIONS_NUMBER_IN_TEST = "questionsNumber";
    public static final String CURRENT_QUESTION_NUMBER = "questionNumber";
    public static final String QUESTIONS_PASSED = "passedQuestions";

    public static final String TEST_ID = "testId";
    public static final String TEST_NAME = "testName";
    public static final String NEW_TESTS = "newTests";
    public static final String POPULAR_TESTS = "popularTests";
    public static final String TEST_LIST = "tests";
    public static final String CURRENT_TEST = "test";
    public static final String PASSING_TEST = "passingTest";
    public static final String TEST_CATEGORY = "testsCategory";
    public static final String TEST_CREATORS = "users";
    public static final String TEST_RESULTS = "results";

    public static final String CATEGORY = "category";
    public static final String CATEGORIES = "categories";

    public static final String SORT_DIRECTION = "direction";
    public static final String SORT_TYPE = "sortType";

    public static final String PAGE_NUMBER = "num";

    public static final String LANGUAGE = "language";

    public static final String PAGINATION = "paginationList";

    public static final String ERROR_MSG = "errorMsg";

    public static final String AUTHORIZATION_ERROR_MSG = "authorizationError";
    public static final String REGISTRATION_ERROR_MSG = "registrationError";
    public static final String CREATE_TEST_ERROR_MSG = "testNameError";
    public static final String CREATE_QUESTION_ERROR_MSG = "questionNameError";

    public static final String DATES = "date";

    public static final String ACTION = "action";

    public static final String RESULT = "result";

    public static final String CORRECT_ANSWER = "correctAnswer";

    private AttributeNames(){}
}
